# -*- coding: utf-8 -*-
"""
CIFAR10 0/1/8 few-shot experiment:
QRF-Net vs parameter-matched CNN, CNN_full, and LeNet.

V7 CIFAR reproducible experiment runner:
1) default to the CIFAR10 0/1/8 setting from the CIFAR notes;
2) use class-wise gated logit residual fusion with a stronger CIFAR frontend;
3) split circuit lr from quantum-head/gate lr so the residual branch learns faster;
...
class QuantumModule(nn.Module):
    def __init__(
        ...
        def circuit(inputs, weights):
            ...
            outs = [qml.expval(qml.PauliZ(i)) for i in range(n_qubits)]
            if readout_mode == "adjacent":
                for i in range(n_qubits - 1):
                    outs.append(qml.expval(qml.PauliZ(i) @ qml.PauliZ(i + 1)))
            elif readout_mode == "all_pairs":
...
def apply_cifar_preset(args):
    ...
    presets = {
        "cifar018": {
            ...
            "logit_residual_lambda_max": 0.35,
            "logit_residual_lambda_init": 0.025,
            "aux_classical_loss_weight": 0.12,
            ...
            "conv_channels": [32, 64],
            "bottleneck_hidden": 128,
            "cnn_feature_dim": 52,
            "cnn_hidden_dim": 64,
            "lr_classical": 8e-4,
            "lr_quantum": 3e-5,
            "lr_quantum_head": 4e-4,
            "weight_decay": 2e-4,
            "grad_clip": 1.0,
            "label_smoothing": 0.03,
            "aug_mode": "medium",
            "score_mode": "balanced",
            "auc_score_weight": 0.25,
            "seeds": [0, 1, 2, 3, 4],
            "summary_path": "results_cifar10_018_qrf_v7_5seeds_summary.json",
        },
        "cifar018_quick": {
            "dataset": "CIFAR10",
            "class_ids": [0, 1, 8],
            ...
            "auc_score_weight": 0.25,
            "seeds": [0, 1, 2, 3, 4],
            "summary_path": "results_cifar10_018_qrf_v7_quick.json",
        },
    }
...
def parse_args():
    ...
    # Experiment
    parser.add_argument("--preset", type=str, default="cifar018",
                        choices=["cifar018", "cifar018_quick", "none"],
                        help="CIFAR preset. Use none to rely only on explicit CLI arguments.")
    parser.add_argument("--model", type=str, default="all",
                        choices=["hybrid", "cnn_control", "cnn_full", "lenet", "all"])
    ...
    parser.add_argument("--summary_path", type=str, default="results_cifar_qrf_v7_5seeds_summary.json")
...
def print_parameter_probe(args):
    ...
    args.device = "cpu"
    rows = []
    for model_name in ["cnn_control", "hybrid", "cnn_full", "lenet"]:
...
def main():
    ...
    model_names = ["cnn_control", "hybrid", "cnn_full", "lenet"] if args.model == "all" else [args.model]
