# -*- coding: utf-8 -*-
"""
MNIST / FashionMNIST / CIFAR10 few-shot experiment:
QRF-Net vs parameter-matched CNN, CNN_full, and LeNet.

V7 reproducible experiment runner:
1) default to the FashionMNIST 2/4/6 setting used in the v6/v7 notes;
2) use class-wise gated logit residual fusion as the default QRF head;
3) add a small auxiliary quantum-head loss so the residual branch learns faster;
4) center quantum residual logits before fusion to focus the correction on class margins;
...
class QuantumModule(nn.Module):
    ...
    def forward(self, x):
        out_device = x.device
        q_inputs = x.to("cpu") if self.force_cpu else x
        q_outs = [self.qlayer(q_inputs[i]) for i in range(q_inputs.shape[0])]
        q_features = self.readout(torch.stack(q_outs, dim=0))
        return q_features.to(out_device)
...
class MobileNetV2SmallBaseline(nn.Module):
    """Small MobileNetV2-style classical CNN baseline."""
    ...
    def __init__(self, num_classes: int, in_channels: int = 1):
        ...
        self.features = nn.Sequential(
            InvertedResidualBlock(16, 16, stride=1, expand_ratio=1),
            InvertedResidualBlock(16, 24, stride=2, expand_ratio=4),
            InvertedResidualBlock(24, 24, stride=1, expand_ratio=4),
            InvertedResidualBlock(24, 32, stride=2, expand_ratio=4),
            InvertedResidualBlock(32, 48, stride=1, expand_ratio=4),
        )
        self.classifier = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),
            nn.Flatten(),
...
class EfficientBlock(nn.Module):
    def __init__(
        ...
        layers.extend([
            ...
            group_norm_for(hidden_dim),
            nn.SiLU(inplace=True),
            SqueezeExcite(hidden_dim, reduction=4),
            nn.Conv2d(hidden_dim, out_channels, kernel_size=1, bias=False),
            group_norm_for(out_channels),
        ])
        self.block = nn.Sequential(*layers)
...
class EfficientNetSmallBaseline(nn.Module):
    """Small EfficientNet-style classical CNN baseline."""
    ...
    def __init__(self, num_classes: int, in_channels: int = 1):
        ...
        self.features = nn.Sequential(
            EfficientBlock(16, 16, stride=1, expand_ratio=1, kernel_size=3),
            EfficientBlock(16, 24, stride=2, expand_ratio=4, kernel_size=3),
            EfficientBlock(24, 24, stride=1, expand_ratio=4, kernel_size=3),
            EfficientBlock(24, 40, stride=2, expand_ratio=4, kernel_size=5),
            EfficientBlock(40, 40, stride=1, expand_ratio=4, kernel_size=5),
        )
...
def build_model(args):
    ...
    if args.model == "hybrid":
        model = HybridCNNQNN(
            ...
            in_channels=in_channels,
            force_quantum_cpu=getattr(args, "force_quantum_cpu", False),
        ).to(args.device)
        if getattr(args, "force_quantum_cpu", False):
            model.qmodule.cpu()
        return model
    ...
    if args.model == "mobilenetv2_small":
        return MobileNetV2SmallBaseline(num_classes=num_classes, in_channels=in_channels).to(args.device)

    if args.model == "efficientnet_small":
        return EfficientNetSmallBaseline(num_classes=num_classes, in_channels=in_channels).to(args.device)
...
def resolve_model_names(model_arg: str) -> List[str]:
    if model_arg == "all":
        return ["cnn_control", "hybrid", "cnn_full", "lenet"]
    if model_arg == "new_classical":
        return ["vgg_small", "resnet_small"]
    if model_arg == "resnet_densenet":
        return ["resnet_small", "densenet_small"]
    if model_arg == "mobile_efficient":
        return ["mobilenetv2_small", "efficientnet_small"]
    if model_arg == "mainstream_classical":
        return [
            "vgg_small", "resnet_small", "densenet_small",
            "mobilenetv2_small", "efficientnet_small",
        ]
    if model_arg == "all_plus_new":
        return [
            "cnn_control", "hybrid", "cnn_full", "lenet",
            "vgg_small", "resnet_small", "densenet_small",
            "mobilenetv2_small", "efficientnet_small",
        ]
    return [model_arg]
...
def parse_args():
    ...
    parser.add_argument("--model", type=str, default="all",
                        choices=[
                            "hybrid", "cnn_control", "cnn_full", "lenet",
                            "vgg_small", "resnet_small", "densenet_small",
                            "mobilenetv2_small", "efficientnet_small",
                            "all", "new_classical", "resnet_densenet",
                            "mobile_efficient", "mainstream_classical", "all_plus_new",
                        ])
    ...
    parser.add_argument(
        "--force_quantum_cpu",
        action="store_true",
        help="Run PennyLane quantum circuit and readout on CPU while keeping other modules on --device.",
    )
    ...
    parser.add_argument("--summary_path", type=str, default="results_fashion246_qrf_v7_5seeds_summary.json")
...
def main():
    args = parse_args()
    if (
        args.model in ("new_classical", "resnet_densenet", "mobile_efficient", "mainstream_classical")
        and args.summary_path == "results_fashion246_qrf_v7_5seeds_summary.json"
    ):
        class_tag = "".join(str(c) for c in args.class_ids)
        args.summary_path = f"results_fashion{class_tag}_{args.model}_v7.json"
